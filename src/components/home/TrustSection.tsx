export function TrustSection() {
    const brands = [
        { name: "ASUS ROG", logo: "https://upload.wikimedia.org/wikipedia/commons/0/00/Republic_Of_Gamers_Logo.svg" },
        ...
        <section className="py-20 relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="relative flex flex-col overflow-hidden">
                    <div className="flex animate-marquee whitespace-nowrap space-x-12 items-center hover:pause">
                        {[...brands, ...brands].map((brand, index) => (
                            <div key={`${brand.name}-${index}`} className="group/item inline-block w-32 md:w-40 opacity-40 hover:opacity-100 transition-all duration-300 transform hover:scale-110 cursor-pointer">
                                <img
                                    src={brand.logo}
                                    alt={brand.name}
                                    className="h-12 w-full object-contain filter brightness-0 invert hover:filter-none hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.5)] transition-all duration-300"
                                />
                            </div>
                        ))}
                    </div>
